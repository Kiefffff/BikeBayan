...
async def station_update(req: EspStationUpdateRequest):
    try:
        for slot_number_str, bike_id in req.slots.items():
            ...
            supabase.table("slots").update({"occupied_bike": bike_id}) \
                .eq("id", slot_id) \
                .execute()
...
async def user_bike_check(req: EspUserBikeCheckRequest):
    try:
        bike_lookup = supabase.table("bikes").select("id") \
        ...
        active_rental = supabase.table("rental").select("bike_id") \
            .eq("user_uin", req.uin) \
...
async def set_user_returned(req: EspReturnedRequest):
    try:
        active_rental = supabase.table("rental").select("id", "bike_id") \
            .eq("user_uin", req.uin) \
            .is_("end_time", "null").execute()
